...
V1 реализован: 3D переключает baseline/сохранённый A; A/B-сравнение остаётся V2.
Live-доступ к gpt-6-luna проверен 23 сентября: короткий запрос успешен, но A1 исчерпал
700 токенов на reasoning, A2 достиг timeout и перешёл в fallback; см. HANDOFF.md.
Настройки генерации для этой модели требуют доработки; demo работает без ключа.
C0 реализован: типы ревизора/сцены и чистые проверки ограничений/результата.
V1 реализован: ui.scene и локальный Three.js показывают baseline/A,
выбор района и снятие выбора null возвращаются в Python; поддержаны версия,
события ошибок и повторный запуск сцены. A/B и слой различий остаются частью V2.
...
citysim/reviewer.py: demo/live-ревизия и объяснение A→B. UI ревизора ещё не подключён.
