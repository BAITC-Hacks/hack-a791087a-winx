...
`validate_scenario` и `simulate` пока явно поднимают `NotImplementedError` (этап E1).
UI не вызывает заглушки и не выдаёт начальное состояние за валидный набор решений.
...
никакого Score для невалидного набора, никакого вызова AI. Заглушка до E1 вместо этого
поднимает `NotImplementedError`. Не ловить её как успешный расчёт.
...
канонизировать по числовому ID меры и ID района. Baseline содержит пустые решения/эффекты.
