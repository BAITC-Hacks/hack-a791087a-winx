...
версии протокола ещё не реализованы. S1 и серверная оркестрация A2 реализованы;
UI ревизора ещё не подключён.
...
Приёмка: 155 Python-тестов, 11 JS-тестов, Chromium и локальная сборка;
...
A2 выполняется параллельно V1. Серверный пакет A2 завершён; напарник подключает UI
ревизора после V1. Весь этап A2 остаётся незавершённым до приёмки UI.
Проверка backend: 25 новых offline-тестов; полный check 144 OK, 4 JS OK, запуск HTTP 200.
...
- [x] Сначала ручное сравнение: посчитать другой валидный набор, сохранить его
...
- [x] Demo берёт до 20 кандидатов из generate_candidates. Live — не более двух
...
- [x] До API-вызова проверить валидность source и constraints. Ответ со сломанным
...
- [x] SDK timeout/max_retries — как в explain_result; общий лимит ревизии 35 секунд,
...
- [x] При недоступности AI выполнить ограниченный demo-поиск и явно указать fallback.
...
- [x] Тесты моками: demo не создаёт SDK; пустые/невалидные ответы; таймаут во втором
...
- [x] Кнопки статуса: improved / cheaper_equal / unchanged / incomplete;
...
**UI готов:** ui/review.py и app.py; ручной B, ограничения, явная ревизия,
status/outcome/warning/checks, принятие B или сохранение A. Новый A/constraints
очищает B/ревизию. Проверки и сценарий — [DEMO.md](DEMO.md).
...
- [x] Одна сцена с переключателем A/B обязательна; две панели с общей камерой —
...
- [x] Пять слоёв: каждый раскрывает конкретные два показателя направления;
  не усреднять их в новый рейтинг. Подсветка «Только различия» по данным A/B.
- [x] Район → показатели → measures/effects в панели; Python фильтрует меры по
  наличию индикатора в effects каталога. Цифры только из сохранённого результата.
- [x] Браузерная приёмка: A==B выглядит одинаково; переключение сохраняет камеру;
...
- [x] Без анимации роста/деградации по кварталам; исключить CDN-запросы после установки. Собрать JS и проверить переключение/различия в браузере; после приёмки отметить V2 в PLAN.

Готово: одна сцена с внешним Python-переключателем baseline/A/B; active_state —
частная подсказка bridge наряду с view_key, ScenePayload C0 не менялся.
«Только различия» ослабляет неизменённые районы по выбранному показателю,
сохраняя их доступность. Контраст и шкала одинаковы; JavaScript только сравнивает
готовые значения. Приёмка: 178 Python-тестов, 16 JS-тестов и браузерные UI-пути;
физический pinch и платный live отдельно не проверялись.
