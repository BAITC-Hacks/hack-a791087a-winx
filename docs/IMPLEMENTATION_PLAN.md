...
План составлен 23.09.2026 по `7069af2`. Проверены 8 тестов P0.
Ни одна новая продуктовая задача ниже пока не реализована.
...
Реализуем тремя пригодными к показу версиями: **MVP → один 3D-макет → A/B с ревизором**.
Каждая сохраняет предыдущий рабочий сценарий. Полный перебор, экспорт и свободный
текст поручений отложены: их отсутствие не блокирует сдачу. Это приоритизация
PROPOSAL, а не отказ от идеи двух будущих состояний.
...
- 17:15 — стоп новым функциям; 17:30 — чистая приёмка; 17:50 — финальный push, GMT+5.
- Коммит/push после каждого принятого пакета. Перед push: pull --rebase --autostash, проверки, push.
...
Это целевые окна с резервом, а не обещание длительности. Не начинать опцию, если
предыдущий контрольный результат не принят.

| Время GMT+5 | @alikhan | @AaaDddmyrza | Контрольный результат |
|---|---|---|---|
| 14:35–15:15 | E1 | U1 на API v1 | Валидатор/симуляция и форма готовы к соединению |
| 15:15–15:35 | E2, сверка чисел I1 | I1 | Эталон 95 / 56.5431 / 0 в UI; ошибки блокируются |
| 15:35–15:55 | C0, начало S1 | A1 | Рабочий MVP без сети; опубликован контракт расширений |
| 15:55–16:25 | S1, тесты ограничений поиска | V1 | Один 3D-макет и проверенные кандидаты отдельно |
| 16:25–16:50 | Помощь со стыковкой и ошибками S1 | A2 | Ревизор + сохранённые A/B в таблицах |
| 16:50–17:15 | Численная приёмка и зависимости | V2 только при готовности; иначе Q1 | Сравнение сцен либо стабильный один макет + таблицы |
| 17:15–17:30 | R1: команды/контракты/зависимости | Q1: тексты, README, DEMO | Функции заморожены, пакет сдачи готов |
| 17:30–17:50 | R2: устраняет выявленные дефекты | R2: чистый клон и запуск по README | Проверенный main и финальный push |

Если к 15:35 не готов I1 — оба занимаются только MVP, 3D/ревизор не начинаются.
Если к 16:25 нет рабочей связи Python↔3D — сохраняем таблицы, V1/V2 исключаем из сдачи.
Если S1/A2 не проходят проверки к 16:50 — показываем ручное сравнение двух валидных
наборов, без кнопки незавершённого ревизора. В 17:15 новые функции не добавляем.
...
- [ ] За 15 минут подтвердить протокол: Python передаёт подписанные районы,
  JS возвращает district_selected. Использовать Components v2 установленного Streamlit:
...
- [ ] Если протокол не заработал в окно V1, не переносить расчёты в JS;
  убрать 3D из сдачи. При успехе `feat: render local 3D district view` и push.
...
- [ ] Без анимации роста/деградации по кварталам; исключить CDN-запросы после
  установки. Собрать JS, проверить в браузере, `feat: compare city futures`, push до 17:15.
...
- [ ] @AaaDddmyrza в 17:30: клон того же репозитория в пустую папку, команды README,
  без .env/ключа, затем demo при отключённой внешней сети после установки.
...
- [ ] Каждый обновляет свой state до 40 строк и статусы PLAN; координатор закрывает
  HANDOFF по выполненным запросам. Финальные коммиты/push до 17:50, без force/history rewrite.

## Отложено за рамки хакатонного обязательного плана

Полный поиск оптимума — @alikhan после замера производительности и отдельного
расширения API. До фильтров 1 407 050 размещений; этот размер не доказывает,
что перебор уложится в срок. Ключ кэша обязан включать dataset/правила/constraints;
только полное завершение объявленного пространства разрешает слово «оптимален».
UI полного поиска, свободный текст поручений, экспорт HTML/презентации и сложные
анимации — @AaaDddmyrza после готового V2. Случайные события сейчас исключены.
