...
from citysim.labels import INDICATOR_NAMES, MEASURE_NAMES, indicator_name, measure_name
from citysim.models import Explanation, SimulationResult


def _explanation_payload(result: SimulationResult) -> dict:
    """Keep the engine result intact and attach ready-to-use explanatory context."""
    payload = asdict(result)
    districts_before = []
    for district in result.districts_after:
        deltas = result.indicator_deltas[district.id]
        indicators = {
            code: value - deltas[code]
            for code, value in district.indicators.items()
        }
        districts_before.append({
            "id": district.id,
            "name": district.name,
            "indicators": indicators,
        })

    def critical_indicators(districts: list[dict]) -> list[dict]:
        return [
            {
                "district_id": district["id"],
                "name": district["name"],
                "indicator": code,
                "indicator_name": indicator_name(code),
                "value": value,
            }
    ...
    payload.update({
        "districts_before": districts_before,
        "critical_indicators_before": critical_indicators(districts_before),
        "critical_indicators_after": critical_indicators([
            {
                "id": district.id,
                "name": district.name,
        ...
        "indicator_names": dict(INDICATOR_NAMES),
        "measure_names": dict(MEASURE_NAMES),
    })
    return payload
...
def _demo_explanation(result: SimulationResult, warning: str | None = None) -> Explanation:
    ...
    if result.is_baseline:
        below_threshold = [
            (district_names.get(district.id, district.id), indicator_name(key))
            for district in result.districts_after
        ...
        text = (
            f"**Что показывает расчёт.** Базовый результат без выбранных мер: итоговый балл (Score) "
            f"{result.after.score:.4f}, взвешенное среднее {result.after.d_avg:.4f}, "
            f"минимальный районный балл {result.after.d_min:.4f}. "
            f"Число показателей ниже 40 (Ncrit): {result.after.n_crit}. "
            "Это только исходный ориентир; он не описывает эффект мер.\n\n"
            ...
            "и сравните итоговый балл, число показателей ниже 40, стоимость и остаток бюджета после расчёта."
        )
    ...
    def critical_pairs(*, before: bool) -> list[str]:
        ...
        for district in result.districts_after:
            ...
            for indicator, after_value in district.indicators.items():
                ...
                if value < 40:
                    pairs.append(f"{name} — {indicator_name(indicator)}")
        return pairs
    ...
    def describe_deltas(items: list[tuple[str, str, float]]) -> str:
        return "; ".join(
            f"{name} — {indicator_name(indicator)} {delta:+.4f}"
            for name, indicator, delta in items
        )
    ...
    for name, indicator, delta in negative:
        ...
        if sources:
            readable_sources = ", ".join(
                " + ".join(f"{part} ({measure_name(part)})" for part in source.split("+"))
                for source in sources
            )
            negative_sources.append(f"{readable_sources}: {name} — {indicator_name(indicator)}")
    source_detail = (
        f" Среди учтённых эффектов есть отрицательные: {'; '.join(negative_sources)}."
    ...
    text = (
        ...
        f"Итоговый балл (Score) до {result.before.score:.4f}, после {result.after.score:.4f} "
        f"(изменение {score_sign}{result.score_delta:.4f}). Число показателей ниже 40 "
        f"(Ncrit): {result.before.n_crit} → {result.after.n_crit}; до: {before_detail}, "
        f"после: {after_detail}.\n\n"
        f"**Сильная сторона.** Наибольший районный балл после расчёта у района "
        f"«{district_names.get(strongest_id, strongest_id)}» ({strongest_score:.4f}). "
        f"Показатели, которые выросли: {positive_detail}.\n\n"
        f"**Риски и компромисс.** Самый низкий районный балл после расчёта у района "
        f"«{weakest_name}» ({weakest_score:.4f}). Показатели, которые снизились: "
        f"{negative_detail}.{source_detail} Изменения показывают фактическое значение после мер "
        "по сравнению с исходным; они не являются отдельными вкладами в итоговый балл.\n\n"
        f"**Следующий шаг.** Проверьте альтернативы для района «{weakest_name}» и сравните их "
        "пересчитанные итоговый балл, число показателей ниже 40, стоимость и остаток бюджета. "
...
def explain_result(
    ...
    *,
    ...
    instructions = (
        ...
        "объясни стоимость и остаток бюджета, Score до и после, Ncrit и конкретные пары район/показатель ниже 40 "
        ...
        " Пиши для человека, который впервые открыл симулятор: короткие предложения, без технического жаргона. "
        "Score называй итоговым баллом города, Ncrit — числом показателей ниже 40; "
        "вместо слова «дельта» пиши «изменение». Объясни, что результат рассчитан по учебной модели. "
        "JSON содержит готовые districts_before и critical_indicators_before/after: используй их напрямую, "
        "не восстанавливай исходные значения и не говори, что их нет. Списки содержат только показатели ниже 40. "
        "Для названий используй indicator_names и measure_names вместе с кодами. "
        + "; ".join(f"{code} — {name}" for code, name in INDICATOR_NAMES.items()) + "."
    )
    ...
    try:
        ...
        response = client.responses.create(
            ...
            input=json.dumps(_explanation_payload(result), ensure_ascii=False),
            store=False,
